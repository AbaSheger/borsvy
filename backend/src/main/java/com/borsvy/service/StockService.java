    ...
    private Map<String, Object> generateMarketBasedNewsSentiment(String symbol) {
        try {
            ...
            if (!stockOpt.isPresent()) {
                ...
                return StockSentimentFallbackFactory.createDefault();
            }

            Map<String, Object> result = StockSentimentFallbackFactory.create(stockOpt.get());
            log.info("Generated market-based sentiment for {}: {}", symbol, result.get("sentiment"));
            return result;
        } catch (Exception e) {
