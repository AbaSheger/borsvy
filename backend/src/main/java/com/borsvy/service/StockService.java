...
public class StockService {
    ...
    public StockDetails getStockDetails(String symbol) throws IOException {
        try {
            ...
            if (quote == null || profile == null) {
                throw new IOException("Failed to fetch data from Finnhub API");
            }

            log.debug("Received company profile for {}: PE={}, MarketCap={}, Industry={}", 
                     symbol, profile.getPe(), profile.getMarketCapitalization(), profile.getFinnhubIndustry());
            ...
            if (marketCapInBillions > 0) {
                details.setMarketCap(marketCapInBillions); // Keep in billions
            }
            
            // Set price and change data from quote
            details.setPrice(quote.getCurrentPrice());
            details.setChange(quote.getChange()); // Set absolute change
            details.setChangePercent(quote.getPercentChange()); // Set percentage change
    ...
    public Optional<Stock> getStockBySymbol(String symbol) {
        try {
            ...
            if (stock.isEmpty()) {
                log.info("Stock not found for symbol: {}, creating new stock", symbol);
                // Create a new stock with default values
                Stock newStock = new Stock();
                newStock.setSymbol(symbol);
                newStock.setPrice(0.0);
                newStock.setChangePercent(0.0);
                newStock.setVolume(0L);
                newStock.setEarningsPerShare(0.0);
                newStock.setDividend(0.0);
                newStock.setSharesOutstanding(0L);
                
                return Optional.of(stockRepository.save(newStock));
            }
    ...
    public List<StockPrice> getHistoricalData(String symbol, String interval) {
        ...
        // If Polygon.io didn't return data, generate synthetic data
        log.info("No Polygon.io data available for {} with interval {}, using synthetic data", symbol, interval);
    ...
    private List<StockPrice> generatePriceHistoryFromCurrentPrice(Stock stock, String interval) {
        ...
        // Generate price history showing the trend from current price
        double price = currentPrice;
        double volatility = Math.abs(stock.getChangePercent()) / 100.0; // Use actual volatility
        ...
        for (int i = dataPoints; i >= 0; i--) {
            ...
            // Generate price based on the stock's actual trend
            double change = volatility * price * (stock.getChangePercent() > 0 ? 0.1 : -0.1);
            price += change;
            
            pricePoint.setPrice(price);
            pricePoint.setVolume(stock.getVolume() / dataPoints); // Distribute volume across time points
